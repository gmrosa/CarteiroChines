...
public class CarteiroChines {
	...
	private void criarMatriz() {
		for (int i = 0; i < tamanho; i++) {
			...
			grafo.addVertice(verticeOrigem);
	...
	private void executarDijkstraParaVerticesDeGrauImpar() {
		...
		List<Character> verticesGrauImpar = new LinkedList<>();
	...
	private List<Character> getVerticesGrauImpar() {
		...
		for (Map.Entry<Character, List<Aresta>> entry : grafo.estruturaGrafo.entrySet()) {
			...
			while (!verticesGrauImparTemp.isEmpty()) {
				for (Map.Entry<Aresta, CaminhoDijkstra> entry : dijkstras.entrySet()) {
					if (dijkstrasTemp.containsKey(entry.getKey())) {
						...
						if (aresta.verticeOrigem == verticesGrauImparTemp.get(0) && //
								verticesGrauImparTemp.contains(aresta.verticeOrigem) && //
								verticesGrauImparTemp.contains(aresta.verticeDestino)) {
							verticesGrauImparTemp.removeIf(c -> c == aresta.verticeOrigem);
							verticesGrauImparTemp.removeIf(c -> c == aresta.verticeDestino);
	...
	private static class Grafo {
		private Map<Character, List<Aresta>> estruturaGrafo = new LinkedHashMap<>();
		private int custoTotal = 0;
		private int custoDuplicadas = 0;

		private void addVertice(Character vertice) {
			estruturaGrafo.put(vertice, new LinkedList<>());
		}

		private void duplicarAresta(Aresta aresta) {
			List<Aresta> arestas = estruturaGrafo.get(aresta.verticeOrigem);
		...
		private void addAresta(Aresta aresta) {
			...
			estruturaGrafo.get(aresta.verticeOrigem).add(aresta);
		}
	...
	private static class Aresta {
		...
		public int hashCode() {
			...
			result = prime * result + verticeOrigem;
			return result;
		}
		...
		public boolean equals(Object obj) {
			...
			if (verticeDestino != other.verticeDestino)
				return false;
			if (verticeOrigem != other.verticeOrigem)
				return false;
			return true;
		}
